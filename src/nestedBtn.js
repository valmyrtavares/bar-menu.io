  ...
  const hasChildItems = () => {
   ...
   return (
    ...
    <div className="nested-btn">
      ...
      {display && childCategory.length > 0 && childCategory.map((childItem, index) => (
      <React.Fragment key={index}>      
      <div>
        <NestedBtn parent={item.parent} item={childItem} menuButton={menuButton} />
    </div>
  </React.Fragment>
))}
    </div>
